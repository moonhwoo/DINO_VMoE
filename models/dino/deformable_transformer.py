...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
                 ...
                 moe_layers=None,
                 moe_num_experts=8,
                 moe_num_selected_experts=2,
                 moe_capacity_factor=1.25,
                 moe_noise_std=1.0,
                 moe_gshard_loss_weight=0.01,
                 moe_importance_loss_weight=1.0,
                 moe_load_loss_weight=1.0,
                 moe_group_images=1,
        ...
        # MoE encoder layers (지정된 layer 위치에 MoE layer 생성)
        moe_encoder_layer = None
        if use_moe and deformable_encoder:
            from models.moe.dino_moe_encoder_layer import DeformableTransformerEncoderMoELayer
            moe_encoder_layer = DeformableTransformerEncoderMoELayer(
                d_model=d_model, d_ffn=dim_feedforward,
                ...
                num_selected_experts=moe_num_selected_experts,
                capacity_factor=moe_capacity_factor,
                noise_std=moe_noise_std,
                gshard_loss_weight=moe_gshard_loss_weight,
                importance_loss_weight=moe_importance_loss_weight,
                load_loss_weight=moe_load_loss_weight,
        ...
        self.encoder = TransformerEncoder(
            ...
            # MoE
            moe_encoder_layer=moe_encoder_layer,
    ...
    def _reset_parameters(self):
        for name, p in self.named_parameters():
            # MoE 모듈은 자체 초기화 유지 (DINOBatchedExperts._init_weights)
            if 'moe' in name or 'router' in name or 'expert' in name:
                continue
            if p.dim() > 1:
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        # Begin Encoder
        #########################################################
        memory, enc_intermediate_output, enc_intermediate_refpoints, moe_metrics = self.encoder(
                src_flatten,
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.moe_layers = moe_layers or []
        # prepare layers — MoE layer가 지정된 위치에는 moe_encoder_layer 사용
        if num_layers > 0:
            if moe_encoder_layer is not None and len(self.moe_layers) > 0:
                layers = []
                for i in range(num_layers):
                    if i in self.moe_layers:
                        layers.append(copy.deepcopy(moe_encoder_layer))
                    else:
                        layers.append(copy.deepcopy(encoder_layer) if not enc_layer_share else encoder_layer)
    ...
    def forward(self, 
            ...
            if not dropflag:
                if self.deformable_encoder:
                    ...
                    if isinstance(result, tuple):
                        ...
                        all_moe_metrics.append(layer_moe_metrics)
                    else:
                        output = result
                else:
        ...
        return output, intermediate_output, intermediate_ref, all_moe_metrics
...
def build_deformable_transformer(args):
    ...
    return DeformableTransformer(
        ...
        moe_num_selected_experts=getattr(args, 'moe_num_selected_experts', 2),
        moe_capacity_factor=getattr(args, 'moe_capacity_factor', 1.25),
        moe_noise_std=getattr(args, 'moe_noise_std', 1.0),
        moe_gshard_loss_weight=getattr(args, 'moe_gshard_loss_weight', 0.0),
        moe_importance_loss_weight=getattr(args, 'moe_importance_loss_weight', 1.0),
        moe_load_loss_weight=getattr(args, 'moe_load_loss_weight', 1.0),
        moe_group_images=getattr(args, 'moe_group_images', 1),
