...
class DINO(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        for l, feat in enumerate(features):
            ...
            srcs.append(self.input_proj[l](src)) #scale별로 입력차원에 맞춰 projection하고 srcs리스트에 추가
            masks.append(mask)
        ...
        if hasattr(self.transformer, '_moe_metrics') and self.transformer._moe_metrics:
            aux_losses = [
                m['auxiliary_loss'] for m in self.transformer._moe_metrics
                if m is not None and 'auxiliary_loss' in m
            ]
            if aux_losses:
                out['moe_auxiliary_loss'] = sum(aux_losses)
            else:
